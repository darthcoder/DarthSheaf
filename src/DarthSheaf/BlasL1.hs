...
-- | rotmg generates Givens rotation parameters (d, c, s, r).
-- Note: This implementation uses basic arithmetic and handles the zero-vector case.
rotmg :: Double -> Double -> Double -> Double -> (Double, Double, Double, Double)
rotmg a b _ d = result
  where
    -- 1. Calculate the norm r = ||[a, b]||_2
    r = sqrt (a * a + b * b)

    result =
      if r == 0.0
        -- Case 1: Zero vector [0, 0]. Avoid division by zero.
        -- We return a default identity-like rotation or simply pass the existing 'd'.
        then (d, 1.0, 0.0, 0.0)
        else
          -- Case 2: Normal vector. Calculate cosine and sine using definition.
          let c_val = a / r -- c = cos(theta)
              s_val = b / r -- s = sin(theta)
           in (d, c_val, s_val, r)

-- Note: We ignore the original c parameter in the inputs as it is redundant
-- given that d is passed through.
