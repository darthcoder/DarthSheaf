...
import DarthSheaf.BlasL1 (Vector, dot)
...
gemv _alpha [] _x _beta _y = []
gemv _ A _ _ _ = error "gemv: length of y must equal number of rows in A"
gemv alpha (row : rows) x beta (yi : ys) = alpha * dot row x + beta * yi : gemv alpha rows x beta ys
